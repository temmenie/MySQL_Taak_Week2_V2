...
namespace MySQL_Taak_Week2_V2
{
    public partial class Form1 : Form
    {
            ...
            try
            {
                using (mySqlConn = OpenMySQLverbinding(mySqlConnStr))
                {
                    if (mySqlConn.State == ConnectionState.Open)
                    {
                        ...
                        mySqlComm.CommandText = "INSERT INTO orders(besteldatum, klantID, productID, bestelHoeveelheid) VALUES(@bestelDatum, @klantID, @productID, @bestelHoeveelheid)";
                        mySqlComm.CommandType = CommandType.Text;
        ...
        private void orderToevoegen_Click(object sender, EventArgs e)
        {
            LaadKlanten();
            LaadProducten();
